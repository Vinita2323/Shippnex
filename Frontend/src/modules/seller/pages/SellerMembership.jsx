...
const SellerMembership = () => {
  ...
  const sellerData = (() => { try { return JSON.parse(localStorage.getItem('shippnex_seller_data') || '{}'); } catch { return {}; } })();
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      ...
      const fn = isRenewal ? membershipService.renewSellerMembership : membershipService.purchaseSellerMembership;
  ...
  if (loading) return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center">
      <div className="text-center"><div className="w-10 h-10 border-4 border-[#ff5500] border-t-transparent rounded-full animate-spin mx-auto mb-3"></div><p className="text-slate-500 font-medium text-sm">Loading...</p></div>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-20">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/seller/dashboard')} className="p-2 rounded-xl hover:bg-slate-100 transition-colors cursor-pointer border-none bg-transparent">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        ...
        {membership?.membershipStatus === 'active' && (
          <div className="bg-gradient-to-br from-[#ff5500] to-[#cc4400] rounded-3xl p-6 text-white shadow-xl shadow-orange-200">
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="text-orange-100 text-xs font-semibold uppercase tracking-wider mb-1">Active Plan</p>
                <h2 className="text-2xl font-black">{membership.planName}</h2>
                <p className="text-orange-200 text-sm font-medium">{durationLabel(membership.durationType)}</p>
              </div>
              <div className="text-right">
                <div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center mb-1"><Crown size={24} /></div>
                {remaining !== null && <p className="text-orange-100 text-xs font-bold">{remaining}d left</p>}
              </div>
            </div>
            <div className="grid grid-cols-3 gap-4 bg-white/10 rounded-2xl p-4 mb-4">
              <div><p className="text-orange-200 text-xs font-semibold">Started</p><p className="text-white text-sm font-bold mt-0.5">{formatDate(membership.startDate)}</p></div>
              <div><p className="text-orange-200 text-xs font-semibold">Expires</p><p className="text-white text-sm font-bold mt-0.5">{formatDate(membership.expiryDate)}</p></div>
              <div><p className="text-orange-200 text-xs font-semibold">Paid</p><p className="text-white text-sm font-bold mt-0.5">₹{membership.priceAtPurchase?.toLocaleString()}</p></div>
            </div>
        ...
        {membership?.membershipStatus === 'expired' && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-5 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-red-700 text-sm">Your <strong>{membership.planName}</strong> expired on {formatDate(membership.expiryDate)}. Renew to regain access to your seller dashboard.</p>
            </div>
          </div>
        )}
        ...
        <div className="flex bg-white border border-slate-200 rounded-2xl p-1 gap-1">
          {[{ id: 'plans', label: 'Choose Plan' }, { id: 'history', label: 'Payment History' }].map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)} className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all cursor-pointer border-none ${activeTab === t.id ? 'bg-[#ff5500] text-white shadow-md' : 'text-slate-500 hover:text-slate-700 bg-transparent'}`}>
              {t.label}
            </button>
          ))}
        </div>
          ...
          plans.length === 0 ? (
            ...
            <div className="grid md:grid-cols-3 gap-5">
              {plans.map((plan) => {
                ...
                return (
                  <div key={plan._id} className={`relative bg-white rounded-2xl border-2 p-6 transition-all hover:shadow-xl hover:-translate-y-1 ${isPopular ? 'border-[#ff5500] shadow-lg shadow-orange-100' : 'border-slate-200'}`}>
                    {isPopular && <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-[#ff5500] text-white text-xs font-black px-4 py-1.5 rounded-full shadow-md">MOST POPULAR</div>}
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-5 ${isPopular ? 'bg-[#ff5500] text-white shadow-md shadow-orange-300' : 'bg-slate-100 text-slate-600'}`}>
                      <PlanIcon size={22} />
                    </div>
                    ...
                    <button
                      ...
                      className={`w-full py-3.5 rounded-xl text-sm font-bold transition-all cursor-pointer border-none disabled:opacity-50 disabled:cursor-not-allowed ${isPopular ? 'bg-[#ff5500] text-white hover:bg-[#e64d00] shadow-lg shadow-orange-200' : 'bg-slate-100 text-slate-800 hover:bg-slate-200'}`}
                    >
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-100 rounded-2xl p-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <p className="text-3xl font-black text-[#ff5500]">₹{selectedPlan.price?.toLocaleString()}</p>
                </div>
              </div>
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['UPI', 'Bank Transfer', 'Cash'].map((m) => (
                    <button key={m} onClick={() => setPaymentMethod(m)} className={`py-2.5 rounded-xl text-xs font-bold border-2 transition-all cursor-pointer ${paymentMethod === m ? 'border-[#ff5500] bg-orange-50 text-[#ff5500]' : 'border-slate-200 text-slate-600 bg-transparent hover:border-slate-300'}`}>{m}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <input type="text" value={paymentRef} onChange={(e) => setPaymentRef(e.target.value)} placeholder="Enter UTR, UPI ref, or transaction ID..." className="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm font-medium outline-none focus:border-[#ff5500] transition-colors" />
              </div>
              ...
              <button onClick={handleSubmitPayment} disabled={submitting} className="w-full py-4 bg-[#ff5500] hover:bg-[#e64d00] text-white font-black rounded-xl transition-colors cursor-pointer disabled:opacity-60 flex items-center justify-center gap-2 border-none shadow-lg shadow-orange-200">
